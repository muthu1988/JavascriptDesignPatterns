...
class MagicSquareGenerator {
    generate(size) {
        ...
        const ver = new Verifier();
        let square;
        do {
            square = [];
            for (let i = 0; i < size; ++i)
                square.push(gen.generate(size));
        } while (!ver.verify(sp.split(square)));
        return square;
    }
}

console.log("MagicSquareGenerator facade hides all methods and generates sq by one call")
console.log(new MagicSquareGenerator().generate(3));